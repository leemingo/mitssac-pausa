...
def initialise_players(team, teamname, params, GKid):
    ...
    """    
    # get player  ids
    player_ids = [c.split('_')[1] for c in team.keys() if   re.match(f'{teamname}_\d*_x', c)]
...
def check_offsides( attacking_players, defending_players, ball_position, GK_numbers, verbose=False, tol=0.2):
    ...
    assert defending_GK_id in [p.id for p in defending_players], "Defending goalkeeper jersey number not found in defending players" # goalkeeper not found (e.g. red card)
...
def default_model_params(time_to_control_veto=3):
    ...
    params['model_converge_tol'] = 0.02 #0.01 # assume convergence when PPCF>0.99 at a given location.
...
def generate_pitch_control_for_event(event, tracking_home, tracking_away, params, GK_numbers, field_dimen = (106.,68.,), n_grid_cells_x = 50, offsides=True):
    ...
    team = event.team
    ball_start_pos = np.array([event.start_x, event.start_y])
    ...
    if team=='Home':
        ...
        defending_players = initialise_players(tracking_home, 'Home', params, GK_numbers[0])
        attacking_players = initialise_players(tracking_away, 'Away', params, GK_numbers[1])
    else:
        assert False, f"Team in possession must be either home or away: {team}"
    ...
    attacking_positions = np.array([p.position for p in attacking_players])
    defending_positions = np.array([p.position for p in defending_players])
    
    attacking_velocities = np.array([p.velocity for p in attacking_players])
    defending_velocities = np.array([p.velocity for p in defending_players])

    params = params.copy()  # to avoid modifying the original params
    params["lambda_att"] = np.array([p.lambda_att for p in attacking_players]) # array of lambda_def for each defending player
    params["lambda_def"] = np.array([p.lambda_def for p in defending_players]) # array of lambda_def for each defending player

    # calculate pitch pitch control model at each location on the pitch
    for i in range(len(ygrid)):
        for j in range(len(xgrid)):            
            target_position = np.array([xgrid[j], ygrid[i]])
            #PPCFa[i,j],PPCFd[i,j] = calculate_pitch_control_at_target1(target_position, attacking_players, defending_players, ball_start_pos, params)
            PPCFa[i, j], PPCFd[i, j] = calculate_pitch_control_at_target(
                target_position, ball_start_pos,
                attacking_positions, attacking_velocities,
                defending_positions, defending_velocities,
                **params
            )
...
def generate_pitch_control_for_trace(tracking_home, tracking_away, params, GK_numbers, field_dimen = (106.,68.,), n_grid_cells_x = 50, offsides=True):
    """ generate_pitch_control_for_trace
...
def generate_pitch_control_for_virtual(event, tracking_home, tracking_away, params, GK_numbers, field_dimen = (106.,68.,), n_grid_cells_x = 50, offsides=True):
    """ generate_pitch_control_for_virtual
    ...
    # Use the position of the passer to avoid unrealistic ball trajectory.
    # ball_start_pos = np.array([tracking_home.ball_x,tracking_home.ball_y])
    if team == 'Home':
        ball_start_pos = np.array([tracking_home[f"{event.player_code}_x"], tracking_home[f"{event.player_code}_y"]])
    elif team == 'Away':
        ball_start_pos = np.array([tracking_away[f"{event.player_code}_x"], tracking_away[f"{event.player_code}_y"]])
    else:
...
@jit(nopython=True)
def calculate_pitch_control_at_target(target_position, ball_start_pos, 
                                  att_position, att_velocity, def_position, def_velocity,
                                  average_ball_speed, max_player_speed, max_player_accel, 
                                  reaction_time, tti_sigma, kappa_def,
                                  lambda_att, lambda_def, lambda_gk,
                                  int_dt, max_int_time, model_converge_tol, 
    ...
    if np.isnan(ball_start_pos[0]): # assume that ball is already at location
        ball_travel_time = 0.0
    else:
        # ball travel time is distance to target position from current ball position divided assumed average ball speed
        ball_travel_time = np.linalg.norm(target_position - ball_start_pos) / average_ball_speed

    # 2. 도착 시간 계산 (simple_time_to_intercept 로직 내장)
    # Attacking players
    n_att = att_position.shape[0]
    tau_att = np.empty(n_att)
    for i in range(n_att):
        r_reaction = att_position[i] + att_velocity[i] * reaction_time
        tau_att[i] = reaction_time + np.linalg.norm(target_position - r_reaction) / max_player_speed
    
    # Defending players
    n_def = def_position.shape[0]
    tau_def = np.empty(n_def)
    for i in range(n_def):
        r_reaction = def_position[i] + def_velocity[i] * reaction_time
        tau_def[i] = reaction_time + np.linalg.norm(target_position - r_reaction) / max_player_speed

    tau_min_att = np.min(tau_att)
    tau_min_def = np.min(tau_def)

    # 3. Solve Equation 3 (적분 불필요 조건 체크)
    ...
    # 4. 적분 루프 (Integration)
    # 관련 있는 선수만 추리기
    att_mask = (tau_att - tau_min_att) < time_to_control_att
    def_mask = (tau_def - tau_min_def) < time_to_control_def
    
    # 마스킹된 인덱스 대신, 루프 안에서 조건문으로 처리 (Numba에서는 이게 더 빠를 수 있음)
    ...
    # 선수별 누적 확률 저장을 위한 배열
    att_PPCF_accum = np.zeros(n_att)
    def_PPCF_accum = np.zeros(n_def)

    while (1 - ptot > model_converge_tol) and (i < dT_array.shape[0]):
        T = dT_array[i]
        
        # Attackers
        for k in range(n_att):
            if att_mask[k]:
                # probability_intercept_ball 로직
                prob = 1 / (1. + np.exp(-np.pi / np.sqrt(3.0) / tti_sigma * (T - tau_att[k])))
                
                dPPCFdT = (1 - PPCFatt[i-1] - PPCFdef[i-1]) * prob * lambda_att[k]
                assert dPPCFdT >= 0, 'Invalid attacking player probability (calculate_pitch_control_at_target)'
                att_PPCF_accum[k] += dPPCFdT * int_dt
                PPCFatt[i] += att_PPCF_accum[k]
        
        # Defenders
        for k in range(n_def):
            if def_mask[k]:
                prob = 1 / (1. + np.exp(-np.pi / np.sqrt(3.0) / tti_sigma * (T - tau_def[k])))
                
                dPPCFdT = (1 - PPCFatt[i-1] - PPCFdef[i-1]) * prob * lambda_def[k]
                assert dPPCFdT >= 0, 'Invalid defending player probability (calculate_pitch_control_at_target)'
                def_PPCF_accum[k] += dPPCFdT * int_dt
                PPCFdef[i] += def_PPCF_accum[k]
...
def calculate_pitch_control_at_target1(target_position, attacking_players, defending_players, ball_start_pos, params):
    """ calculate_pitch_control_at_target
