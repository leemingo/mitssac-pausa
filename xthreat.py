...
def _get_cell_indexes(
    ...
    xi = x.divide(field_length).multiply(l)
    yj = y.divide(field_width).multiply(w)
...
def _count(x, y, l: int = N, w: int = M) -> npt.NDArray[np.int_]:
...
class ExpectedThreat:
    ...
    def interpolator(
        ...
        cell_length = field_length / self.l
        cell_width = field_width / self.w

        x = np.arange(0.0, field_length, cell_length) + 0.5 * cell_length
        y = np.arange(0.0, field_width, cell_width) + 0.5 * cell_width
    ...
    def rate(
        ...
        if not use_interpolation:
            ...
            l = int(field_length * 10)
            w = int(field_width * 10)
            xs = np.linspace(0, field_length, l)
            ys = np.linspace(0, field_width, w)
            grid = interp(xs, ys)
